    ...
    if ($userData['healthStatus'] == '1' && !isset($_REQUEST['health_conditions'])){
        ...
        if ($key !== false && strstr($ineligibleDisesase, $existingDiseases) !== false) {
            ...
            exit();
                ...
                $donationCenterQuery = "SELECT * FROM `donationcenter`WHERE `D_NAME` = '$donationCenter'";
                $result = $conn -> query($donationCenterQuery);
                $donationCenterData = $result->fetch_assoc();

                $donationCenterId = $donationCenterData['D_ID'];
